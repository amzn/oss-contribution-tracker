...
export default class ProjectPicker extends SurveyStep<State, Data> {
  ...
  fetchProjects = async () => {
    ...
    let projects = projectData.projectList
      .filter((project) => {
  ...
  ready = () => {
    return (
      this.props.ownData.project_id != null && this.props.ownData.project_id.length > 0
    );
  }
    ...
    return (
      <div>
        <div className="form-group">
          ...
          <select name="project" className="form-control" onChange={this.selectProject}>
            <option value="" />
            {this.state.projects.map(proj => <option value={proj.project_id} key={proj.project_id}>
              {proj.project_name}
            </option>)}
          </select>
        </div>
        <div className="form-group">
          <button className="btn btn-outline-dark" onClick={() => this.props.changeStep('project-details')}>
            The project isn't listed here
          </button>
        </div>
        <button className="btn btn-primary btn-lg" onClick={() => this.props.changeStep('contrib-type')} disabled={!this.ready()}>
