...
export function getClaProjectNames() {
  return pg().query(
    'select distinct project_name from cla order by project_name asc'
  );
}

// Add a new cla to the DB
export async function pushNewCla(
  project_name,
  signatory_name,
  approver_name,
  contact_name,
...
export async function getSingleCla(projetId) {
  return await pg().query('select * from cla where project_id = $1', [
    projetId,
  ]);
}
...
// Update query
export async function updateSingleCla(
  project_name,
  signatory_name,
  approver_name,
  contact_name,
